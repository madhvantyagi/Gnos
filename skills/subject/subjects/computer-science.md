...
Use [Excalidraw](../../lesson-design/references/excalidraw.md) for structure and boundaries,
[Manim](../../manim-voice-animation/SKILL.md) for narrated state changes, and
[Pinepaper](../../lesson-design/references/pinepaper.md) for linked or interactive diagrams.
...
- **Show and check:** Use Excalidraw for arrays, nodes, references, and ownership. Trace
  insertion or deletion beside code. Animate a rebalance when intermediate links explain
  the operation. Check the invariant afterward.
...
- **Show and check:** Trace one input first. Use Manim for a narrated search, sort, or
  dynamic-programming sequence. Keep state and code synchronized. Use Pinepaper for
  stepping through a graph. Derive cost from the trace and test a changed input.
...
- **Show and check:** A sequence diagram compares success and delayed response. Logs
  connect it to implementation. Control delay, loss, or retry to test a guarantee.
...
- **Show and check:** Use tables for exact rows and Excalidraw for keys and relations.
...
- **Research:** Inspect the engine documentation for the chosen isolation level and
  ...
  storage.
- **Show and check:** Use Excalidraw transaction lanes or index pages. Use Manim when
...
- **Show and check:** Use Excalidraw for services, interfaces, and request lanes. Pair
  the diagram with concrete requests and responses. Trace a timeout or invalid input.
  Animate only when timing explains a behavior the static sequence hides.
...
- **Show and check:** Use Excalidraw for trust boundaries and data flow. Pair it with
