...
## 5. Narrated-scene checklist (timeline, shared state, cleanup)

- `LinearTransformationScene.apply_matrix` plays outside the `CuePlayer`
  timeline, so narrated scenes should drive the transform with a `ValueTracker`
  (for example, M(t) from identity to A) and pass every animation through
  `player.play(cue_id, ...)` plus `finish(...)`. Keep the matrix header fixed
  (the foreground-pin equivalent: never transform it).
- Tie `DecimalNumber` readouts (image coordinates, determinant) to the same
  tracker as the arrows and unit square; never rebuild `MathTex` per frame.
  Labels follow with a light `next_to` updater and are cleared at the end.
- Keep the camera stable and check 16:9 bounds (x in [-7.11, 7.11],
  y in [-4.0, 4.0]). Ghost vectors record the pre-image so before/after stays
  comparable.
