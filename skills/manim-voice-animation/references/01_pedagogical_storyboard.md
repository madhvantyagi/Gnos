...
Validate the storyboard against `templates/storyboard_schema.json` before
spending time on narration: `python3 skills/manim-voice-animation/scripts/linter.py
path/to/storyboard.json` checks scene and cue ID shape and uniqueness,
nonempty narration, positive durations, and the teaching fields below, mirroring
`scripts/voice_synthesizer.py`. The linter also checks scene code for CuePlayer
coverage (one `play` per cue plus `finish`), updater cleanup, per-frame
`MathTex` rebuilds, and label bounds.

```json
{
  "title": "Derivative as Tangent Slope",
  "concept_id": "calc-derivative-tangent",
  "prerequisite": "Learner can compute the slope between two points.",
  "success_check": "Learner predicts the tangent slope sign after halving delta x.",
  "aspect_ratio": "16:9",
  "scenes": [
    {
      "id": "scene_1",
      ...
      "narration_cues": [
        {
          ...
          "visible_objects": ["curve", "secant_line"],
          "change": "The curve and initial secant appear.",
          "duration": 5.0
        },
        {
          "cue_id": "c2",
          "text": "By picking a second nearby point, we draw a secant line connecting them.",
          "visible_objects": ["second_point", "secant_line"],
          "change": "A second point anchors the secant through both points.",
          "duration": 5.0
        },
        {
          "cue_id": "c3",
          "text": "As delta x shrinks to zero, the secant line morphs directly into the tangent line.",
          "visible_objects": ["secant_line", "tangent_line"],
          "change": "The secant morphs into the tangent as delta x tends to zero.",
          "duration": 6.0,
          "pause_after": 0.8
        }
      ]
...
`sync_target` remains accepted as a legacy free-text alias, but new
storyboards should prefer `visible_objects` plus `change` so each cue names its
objects and the state change the learner must inspect.
