...
## 4. Teaching checklist: causality, numbers, cues, cleanup

- Show the cause with the motion: draw the force arrow from the same state
  that moves the body (`F = -k*x - c*v` from `sim_state`, not a second copy).
  The learner should see the arrow flip sign at each extreme while velocity
  and position follow it.
- Tie every number to simulation state: one `DecimalNumber` per quantity,
  updated in place from `sim_state` (or a `ValueTracker`). Never rebuild
  `MathTex` every frame; clear each number updater with the motion updater.
- Map each template block to one narration cue (`c1` setup, `c2` force law,
  `c3` run, `c4` freeze). The narrated form replaces each `self.play` block
  with one `player.play(cue_id, ...)` keeping `run_time` inside the cue, and
  the `c3` run uses `player.play("c3")` with no animation args while the
  attached updater evolves the state.
- Keep the frame readable: pin the x / v / F panel away from the `Title`,
  keep the full swing inside the declared `Axes` range at 16:9, and detach
  all updaters before the closing changed case (for example, zero damping).
