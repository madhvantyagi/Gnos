...
waits for any remaining clip time. It rejects unknown cue IDs, reused cues,
and animation durations longer than the cue; rewrite the narration or divide
the choreography rather than silently cutting speech. `pause_after` adds
authored reflection time after the recorded cue end. Because each clip is
attached at the current scene time, earlier pauses cannot drift later words
away from their visual events. `finish` rejects unplayed cues and exports
actual cue starts to SRT and timing JSON from the same measured timings.
...
null` with manifest `mode: silent-preview` and `silent: true`. They exercise
the same visual sequence without claiming speech exists. Local recordings
named `SceneID_cue-id.mp3` are measured from the real files; a missing file
fails loudly and measured durations feed back to the timing manifest plus
preview SRTs. Synthesized recordings use fresh per-run directories; a failed
synthesis writes no new manifest, leaves the previous successful manifest and
its clips usable, and reports the provider error without hiding it. Only
storyboard cue text is sent to the provider; never send learner records.
...
with extra pauses unless those pauses are also represented in the audio. Never
mux again after cue-timed audio is already in the scene: the mux step refuses
a video that already contains an audio stream.
