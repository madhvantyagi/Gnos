...
sequence. Videos with cue-timed audio already embedded (CuePlayer.add_sound)
are rejected here: muxing again would replace synced narration and reintroduce
drift.
"""
import argparse
...
def mux_audio_video(video_path, audio_path, output_path):
    ...
    if any(s['type'] == 'audio' for s in visual['streams']):
        raise ValueError('Video already contains audio (cue-timed narration is already in the scene); '
                         'refusing to mux again. Use the Manim render directly.')
